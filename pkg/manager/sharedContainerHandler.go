...
func (m *Manager) ListSharedContainers() ([]Element, error) {
	...
	tmpContainers, err := cache.RetrieveSharedContainers(walletAddress, m.selectedNetwork.ID)
	if err != nil {
		return nil, err
	}
...
func (m *Manager) listSharedContainerObjectsAsync(containerID string) ([]Element, error) {
	...
	for _, v := range list {
		...
		go func(vID oid.ID) {
			...
			//store in database
			if err = cache.StoreSharedObject(walletAddress, m.selectedNetwork.ID, vID.String(), str); err != nil {
...
func (m *Manager) ListSharedContainerObjects(containerID string, synchronised bool) ([]Element, error) {
	...
	tmpObjects, err := cache.RetrieveSharedObjects(walletAddress, m.selectedNetwork.ID)
	if err != nil {
		return nil, err
	}
...
func (m *Manager) AddSharedContainer(containerID string) error {
	...
	if err := cache.StoreSharedContainer(walletAddress, m.selectedNetwork.ID, containerID, marshal); err != nil {
